#!/usr/bin/env bash
# GreenNode AgentBase â€” Shared functions for all resource scripts.
...
#   REDACT_FIELDS  â€” comma-separated fields to redact in stdout output
#   SAVE_AS        â€” save raw response to this file instead of last_response.json
...
api_call() {
  ...
  if [ "$VERBOSE" = true ]; then
    echo ">>> $method $url" >&2
    [ -n "$body" ] && echo ">>> Body: (redacted â€” use --dry-run to see curl command)" >&2
  fi
  ...
  # Handle 401 â€” retry once with fresh token
