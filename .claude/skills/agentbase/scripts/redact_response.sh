...
# If jq parsing fails (non-JSON response), do NOT echo raw text â€” it may contain secrets
if echo "$RESPONSE" | jq -e '.' &>/dev/null; then
  echo "$RESPONSE" | jq "$JQ_FILTER" 2>/dev/null || echo "$RESPONSE"
else
  echo "(non-JSON response saved to $SAVE_RAW â€” not displayed due to potential sensitive content)"
fi
